...
/*
void  zkp_schnorr_proof_to_bytes   (uint8_t **bytes, uint64_t *byte_len, const zkp_schnorr_proof_t *proof, const ec_group_t ec, int move_to_snd)
{
  ...
  group_elem_to_bytes(&set_bytes, GROUP_ELEMENT_BYTES, proof->A, ec, 1);
...
void  zkp_schnorr_proof_from_bytes (zkp_schnorr_proof_t *proof, uint8_t **bytes, uint64_t *byte_len, const ec_group_t ec, int move_to_end)
{
  ...
  group_elem_from_bytes(proof->A, &read_bytes, GROUP_ELEMENT_BYTES, ec, 1);
