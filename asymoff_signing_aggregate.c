...
asymoff_sign_agg_data_t **asymoff_signing_aggregate_parties_new(asymoff_party_data_t ** parties, scalar_t *msgs)  {
  ...
  for (uint64_t i = 0; i < num_parties; ++i) {
    ...
    party->bitlen_plus_1_num_parties = bitlen_plus_1_num_parties;
    ...
    if (i == 0) {
      ...
      party->in_aggregate_msg_offline = calloc(num_parties, sizeof(asymoff_sign_agg_msg_offline_t));
      ...
      party->pi_sig_anchor          = zkp_well_formed_signature_new(ec);
      party->pi_sig_local_agg_proof = zkp_well_formed_signature_new(ec);
      party->pi_sig_agg_proof       = zkp_well_formed_signature_new(ec);
      ...
      for (uint64_t p = 0; p < PACKING_SIZE; ++p) {
        party->pi_sig_anchor_secret.alpha[p]    = scalar_new();
        party->pi_sig_anchor_secret.beta[p]     = scalar_new();
        party->pi_sig_anchor_secret.delta_LB[p]  = scalar_new();
        party->pi_sig_anchor_secret.delta_UA[p]  = scalar_new();
      }
...
void asymoff_signing_aggregate_parties_free(asymoff_sign_agg_data_t **presign_parties) {
...
int asymoff_signing_aggregate_execute_round_1(asymoff_sign_agg_data_t *party) {
  ...
  for (uint64_t l = 0; l < num_sigs/PACKING_SIZE; ++l) {
    ...
    pack_plaintexts(packed, &party->pi_sig_anchor_secret.mu[PACKING_SIZE*l], party->paillier_pub[0]->N, 1);
    ...
    pack_plaintexts(packed, &party->pi_sig_anchor_secret.xi[PACKING_SIZE*l], party->paillier_pub[0]->N, 1);
...
uint64_t asymoff_signing_aggregate_send_msg_3(asymoff_sign_agg_data_t *sender, asymoff_sign_agg_data_t *receiver) {
  ...
  return zkp_el_gamal_dlog_proof_bytelen(sender->num_sigs, 1) + zkp_double_el_gamal_proof_bytelen() + zkp_well_formed_signature_proof_bytelen();
}
...
int asymoff_signing_aggregate_execute_final (asymoff_sign_agg_data_t *party) {
  ...
  if (zkp_el_gamal_dlog_verify(party->pi_eph_agg_proof, &party->pi_eph_agg_public, party->aux, 1) != 1) {
  ...
  if (zkp_well_formed_signature_verify(party->pi_sig_agg_proof, &party->pi_sig_agg_public, party->aux, party->bitlen_plus_1_num_parties) != 1) {
...
uint64_t asymoff_signing_send_msg_offline(asymoff_sign_agg_data_t *sender, asymoff_sign_agg_data_t *receiver) {
  ...
  asymoff_sign_agg_msg_offline_t *in_agg_msg_4 = &receiver->in_aggregate_msg_offline[sender->i];

  in_agg_msg_4->pi_eph_agg_proof = sender->pi_eph_agg_proof;
  in_agg_msg_4->pi_chi_agg_proof = sender->pi_chi_agg_proof;
  in_agg_msg_4->pi_sig_agg_proof = sender->pi_sig_agg_proof;

  in_agg_msg_4->R        = sender->R;
  in_agg_msg_4->joint_V1 = sender->joint_V1;
  in_agg_msg_4->joint_V2 = sender->joint_V2;
  in_agg_msg_4->packed_S = sender->pi_sig_agg_public.packed_S;
  in_agg_msg_4->packed_Z = sender->pi_sig_agg_public.packed_Z;
...
int asymoff_signing_aggregate_execute_offline (asymoff_sign_agg_data_t *party, uint64_t sender_index, scalar_t *signature_s) {
  if (party->i != 0) return 1;
  ...
  assert(num_parties > sender_index);
  
  asymoff_sign_agg_msg_offline_t *msg = &party->in_aggregate_msg_offline[sender_index];
  ...
  if (zkp_el_gamal_dlog_verify(msg->pi_eph_agg_proof, &pi_eph_agg_public, party->aux, 1) != 1) {
    printf("Aggregated ZKP Ephemeral El Gamal DLog verification failed.\n");
    return 1;
  }
  ...
  if (zkp_well_formed_signature_verify(msg->pi_sig_agg_proof, &pi_sig_agg_public, party->aux, party->bitlen_plus_1_num_parties) != 1) {
    printf("Aggregated ZKP Well Formed Signature verification failed.\n");
    return 1;
  }
  ...
  for (uint64_t l = 0; l < num_sigs/PACKING_SIZE; ++l) {
    ...
    unpack_plaintexts(&party->signature_sigma[PACKING_SIZE*l], dec_packed_sigma);
  }
